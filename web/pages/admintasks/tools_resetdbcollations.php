...
<?php
    ...
    if (isset($_POST['confirm']))
    {
		...
		if ($_POST['printonly'] > 0) {
			...
			$rs_tables = $db->query('SHOW TABLES') or die(mysqli_error($link));
			while ($row_tables = $db->fetch_row($rs_tables))
			{
				$table = mysql_real_escape_string($link, $row_tables[0]);
				echo "ALTER TABLE `$table` DEFAULT CHARACTER SET $character_set;<br>";
				$rs = $db->query("SHOW FULL FIELDS FROM `$table` WHERE collation is not null AND collation <> 'utf8_general_ci'") or die(mysqli_error($link));
				while ($row=mysql_fetch_assoc($rs))
				{
					...
					if ( $row['Default'] === NULL && $nullable = ' NOT NULL ')
						...
						$default = " DEFAULT '".mysql_real_escape_string($link, $row['Default'])."'";
					else
						$default = '';
					
					$field = mysql_real_escape_string($link, $row['Field']);
			...
			$db->query("ALTER DATABASE `".DB_NAME."` DEFAULT CHARACTER SET $character_set COLLATE $convert_to;")or die(mysqli_error($link));
			echo 'OK';
			$rs_tables = $db->query('SHOW TABLES') or die(mysqli_error($link));
			while ($row_tables = $db->fetch_row($rs_tables))
			{
				...
				while ($row=mysqli_fetch_assoc($rs))
				{
					...
					if ( $row['Default'] === NULL && $nullable = ' NOT NULL ')
						...
						$default = " DEFAULT '".mysqli_real_escape_string($link, $row['Default'])."'";
					else
						$default = '';
					
					$field = mysqli_real_escape_string($link, $row['Field']);
